...
import apiv1.dentistsmile_core as stcore
from dentistsmile_backend import secret_settings
...
@token_auth(roles=['user', 'admin'], get_user=True)
def dentistsmile_trace(user: ta_models.UserAuthentication, request: WSGIRequest):
    if request.method=='GET':
        profile = models.UserProfile.objects.get(authentication=user)
        child = models.Children.objects.get(id=int(request.GET['child_id']))
        return JsonResponse({'all_traces': [model_to_dict(i) for i in models.DentistsmileTrace.objects.filter(user=profile, children=child)]})
    elif request.method=='POST':
        data = json.loads(request.body)
        saved_traces = []
        for trace in data['all_traces']:
            ...
            growth_level, z_score = stcore.dentistsmile_classification(children.gender, age_month, float(trace['height']))

            trace_object = models.DentistsmileTrace(
                user=profile,
                week=trace['week'],
                height=trace['height'],
                weight=trace['weight'],
        ...
        for trace in data['all_traces']:
            ...
            trace_object = models.DentistsmileTrace.objects.get(user=profile, week=trace['week'], children=models.Children.objects.get(id=data['child_id']))
        ...
        for trace_id in data['to_delete_ids']:
            trace = models.DentistsmileTrace.objects.filter(id=trace_id)
            if len(trace)==0:
                continue
            trace = trace[0]
            trace.delete()
...
def children_management(auth: ta_models.UserAuthentication, request: WSGIRequest):
    ...
    if request.method == 'GET':
        ...
        for child in all_childrens:
            last_child_trace = models.DentistsmileTrace.objects.filter(children=child).order_by('-week')
...
def data_stats(request: WSGIRequest):
    ...
    for child in all_childs:
        the_traces = models.DentistsmileTrace.objects.filter(children=child).order_by('-week')
        child = child.id
        if len(the_traces)==0:
    ...
    # dentistsmile bar, percentage, trends
    growth_agemonth_dentistsmile = dict()
    ...
    for child_id, trace in childs_trace.items():
        ...
        # dentistsmile bar
        if not (growth_level in growth_agemonth_dentistsmile):
            growth_agemonth_dentistsmile[int(growth_level)] = dict()
        if not age_in_month in growth_agemonth_dentistsmile[growth_level]:
            growth_agemonth_dentistsmile[growth_level][age_in_month] = 1
        else:
            growth_agemonth_dentistsmile[growth_level][age_in_month] += 1
    ...
    growth_agemonth_dentistsmile = {key1:{key:growth_agemonth_dentistsmile[key1][key] for key in sorted(growth_agemonth_dentistsmile[key1].keys())} for key1 in growth_agemonth_dentistsmile}
    
    return JsonResponse({'child_traces': childs_trace, 'growth_agemonth_dentistsmile': growth_agemonth_dentistsmile, 'availble_trace': availble_trace, 'growth_group_count': growth_group_count, 'growth_group_total': sum(growth_group_count.values()), 'growth_date_trends': growth_date_trends})
