...
def generate_childs_parents(row_count):
    ...
    for index, row in df.iterrows():
        ...
        try:
            growth_level, z_score = stcore.dentistsmile_classification(row['sex'], int(week/4), height)
        except:
            print('error: ', int(week/4))
            break
        trace = DentistsmileTrace(user=parent, week=week, height=height, weight=random.randrange(1, 35), age_day=week*4, exclusive_asi=True, disease_history=False, immunization_history=False, children=child, z_score=z_score, growth_level=growth_level)
        trace.save()
    # break
# print('success')
# all_childrens = 